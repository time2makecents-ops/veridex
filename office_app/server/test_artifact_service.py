...
class ArtifactServiceTests(unittest.TestCase):
    def test_crud_flow(self) -> None:
        ...
        try:
            ...
            self.assertEqual(deleted["artifact_id"], created["artifact_id"])
            self.assertEqual(service.list_artifacts("ws_test", include_archived=True), [])
            ...
            self.assertEqual({row["workspace_id"] for row in global_rows}, {"ws_other"})
