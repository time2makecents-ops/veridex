...
def handle_archive_store_text(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    append_incident(
        ...
        output_ref=record["stored_filename"],
        evidence_path=str(archive_service.artifacts_dir(workspace_id)),
    ...
    return {
        ...
        "content": [{"type": "text", "text": f"Records Archive stored {record['display_name']} as {record['artifact_id']}."}],
    }


def handle_archive_list(args: Dict[str, Any]) -> Dict[str, Any]:
    workspace_id = args["workspace_id"]
    rows = archive_service.list_artifacts(workspace_id)
    return {
